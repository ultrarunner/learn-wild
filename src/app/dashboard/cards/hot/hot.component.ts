...
@Component({
  ...
  template: `
      <mat-card style="min-width: 280px; max-width: 412px; margin: 5px;" (click)="onSelected()">
        <mat-card-header>
          <div matCardAvatar><img src="/assets/borntolearnwild.png" style="margin-right: 10px; width:30px; height: 30px;"/></div>
        ...
        <mat-card-content>
          ...
          <mat-list-item *ngFor="let item of nytItems">
            <div class="mat-list-item-text" layout="column">                   
              <button mat-icon-button (click)="onOpenNytDialog(item)" matTooltip="View Summary">
                <mat-icon [style.color]="item.today ? '#b62025' : 'white'">info</mat-icon>
              </button>
              <button mat-icon-button (click)='onOpenNytLink(item)' matTooltip="Open in New Window" matTooltipPosition="above">
          ...
          <mat-list-item *ngFor="let item of rssItems">
            <div class="mat-list-item-text" layout="column">                     
              <button mat-icon-button (click)="onOpenRssDialog(item)" matTooltip="View Summary">
                <mat-icon [style.color]="item.today ? '#b62025' : 'white'">info</mat-icon>
              </button>
              <button mat-icon-button (click)='onOpenRssLink(item)' matTooltip="Open in New Window" matTooltipPosition="above">
                <mat-icon>open_in_new</mat-icon>
              </button>         
              {{item.title}}
            </div>
          ...
          <mat-list-item *ngFor="let item of podcastItems">
            <div class="mat-list-item-text" layout="column">                     
              ...
              <button matTooltip="Play Audio" mat-icon-button *ngIf="item.enclosure.link != null" (click)="onSelectMedia(item.enclosure)">
